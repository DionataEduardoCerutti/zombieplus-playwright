...
test.beforeAll(async () => {//Executa uma única vez
    await executeSQL(`DELETE FROM tvshows WHERE title NOT IN ('Fear the Walking Dead', 'Reality Z')`)
})
...
test('deve cadastrar uma nova série', async ({ page }) => {
    ...
    //await tvshowsPage.create('Nome da série', 'Sinopse da série', 'Netflix', i_temporadas, '1980')
...
test('deve poder remover uma série', async ({ page, request }) => { 
    await executeSQL(`DELETE FROM tvshows WHERE title NOT IN ('Fear the Walking Dead')`)//Deleta todas as séries exceto 1 do outro teste
...
test('não deve cadastrar uma série já existe', async ({ page, request }) => { 
    await executeSQL(`DELETE FROM tvshows WHERE title NOT IN ('Reality Z')`)//Deleta todas as séries exceto 1 do outro teste
    const tvshow = data.duplicate
    await request.api.postTvshow(tvshow)//Não cadastra porque já ficou fixo no banco o filme

    await page.login.do('admin@zombieplus.com', 'pwd123', 'Admin')
    //por o playwright ser mult-tread, executar vários testes de uma única vez pode fallhar este teste porque
    //após criar o filme no passo posttvshow um outro teste pode deletar o filme e com isso falhar a validação abaixo.
    //uma alternativa seria colocar no "await executeSQL(`DELETE FROM tvshows`)" do beforeAll um where com este filme
